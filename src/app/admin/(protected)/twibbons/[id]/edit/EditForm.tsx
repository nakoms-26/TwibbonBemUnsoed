...
export default function EditForm({ twibbon }: { twibbon: any }) {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // form will naturally submit via action prop
  };
      ...
      <div className="bg-white rounded-[2rem] border-2 border-gray-100 shadow-xl overflow-hidden relative">
        ...
        <form action={updateTwibbon} onSubmit={handleSubmit} className="p-8 md:p-12 space-y-10">
          ...
          <div className="space-y-8">
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <label htmlFor="title" className="block text-xs font-black uppercase tracking-widest text-gray-900 mb-2">
                ...
                <div>
                  <label htmlFor="slug" className="block text-xs font-black uppercase tracking-widest text-gray-900 mb-2">
            ...
            <div>
              <label htmlFor="description" className="block text-xs font-black uppercase tracking-widest text-gray-900 mb-2">
                Caption
              </label>
              <textarea
                id="description"
                name="description"
                rows={3}
                defaultValue={twibbon.description || ""}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <label className={`relative flex items-center justify-center p-4 border-2 rounded-xl cursor-pointer transition-all ${type === "IMAGE" ? "border-[#0038FF] bg-blue-50" : "border-gray-200 hover:border-gray-300"}`}>
                    <input
                      type="radio"
                      name="type"
                      value="IMAGE"
                      ...
                      className="sr-only"
                    />
                    <span className={`text-sm font-black uppercase tracking-widest ${type === "IMAGE" ? "text-[#0038FF]" : "text-gray-400"}`}>
                      GAMBAR (STATIC)
                    </span>
                    {type === "IMAGE" && <div className="absolute top-2 right-2 w-3 h-3 bg-[#0038FF] rounded-full" />}
                  </label>
                  <label className={`relative flex items-center justify-center p-4 border-2 rounded-xl cursor-pointer transition-all ${type === "VIDEO" ? "border-[#0038FF] bg-blue-50" : "border-gray-200 hover:border-gray-300"}`}>
                    <input
                      type="radio"
                      name="type"
                      value="VIDEO"
                      ...
                      className="sr-only"
                    />
                    <span className={`text-sm font-black uppercase tracking-widest ${type === "VIDEO" ? "text-[#0038FF]" : "text-gray-400"}`}>
                      VIDEO (WEBGL)
                    </span>
                    {type === "VIDEO" && <div className="absolute top-2 right-2 w-3 h-3 bg-[#0038FF] rounded-full" />}
                  </label>
                </div>
              </div>
              ...
              <div>
                ...
                <label className="relative flex items-center p-4 border-2 border-gray-200 rounded-xl cursor-pointer hover:border-gray-300 transition-all bg-gray-50 h-[60px]">
                  <input
                    type="checkbox"
                    name="isActive"
                    defaultChecked={twibbon.isActive}
          ...
          <div className="border-t-2 border-gray-100 pt-10">
            ...
            <p className="text-sm font-bold text-gray-500 mb-6">Biarkan kosong jika tidak ingin mengubah file saat ini.</p>
            
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="bg-blue-50/50 p-6 rounded-2xl border-2 border-blue-100 border-dashed">
                <label htmlFor="layerFile" className="block text-sm font-black uppercase tracking-widest text-[#0038FF] mb-3 flex items-center gap-2">
                ...
                {twibbon.overlayFile && (
                   <p className="mt-2 text-xs font-black text-[#0038FF] truncate">Saat ini: {twibbon.overlayFile.split('/').pop()}</p>
                )}
              </div>

              <div className="bg-gray-50 p-6 rounded-2xl border-2 border-gray-200 border-dashed">
                <label htmlFor="thumbnailFile" className="block text-sm font-black uppercase tracking-widest text-gray-900 mb-3 flex items-center gap-2">
                ...
                {twibbon.thumbnail && (
                   <p className="mt-2 text-xs font-black text-gray-700 truncate">Saat ini: {twibbon.thumbnail.split('/').pop()}</p>
                )}
              </div>
            </div>
          </div>
